...
def update_db():
    ...
    def processTraits():
        ...
        for row in data:
            ...
            traitDataCn = row['traits_zh']

            for skillData in sorted(traitData.get('skills', ()), key=lambda i: i['header']):
                typeLines.append(convertSection(skillData))
            for skillDataCn in sorted(traitDataCn.get('skills', ()), key=lambda i: i['header']):
                typeLinesCn.append(convertSection(skillDataCn))
            if 'role' in traitData:
                typeLines.append(convertSection(traitData['role']))
            if 'role' in traitDataCn:
                typeLinesCn.append(convertSection(traitDataCn['role']))
            if 'misc' in traitData:
                typeLines.append(convertSection(traitData['misc']))
            if 'misc' in traitDataCn:
                typeLinesCn.append(convertSection(traitDataCn['misc']))
            traitLine = '<br />\n<br />\n'.join(typeLines)
            traitLineCn = '<br />\n<br />\n'.join(typeLinesCn)

            newRow = {'typeID': typeId, 'traitText': traitLine, 'traitTextCn': traitLineCn}
