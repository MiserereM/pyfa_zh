...
class ShipBrowser(wx.Panel):
    ...
    def stage1(self, event):
        ...
        for ship in self.categoryList:
            if self.filterShipsWithNoFits and not self.categoryFitCache[ship.ID]:
                ...
                self.lpane.AddWidget(CategoryItem(self.lpane, ship.ID, (ship.nameCn, 0)))
    ...
    def stage2Callback(self, data):
        ...
        for ship in ships:
            ...
            shipTrait = ship.traits.traitTextCn if (ship.traits is not None) else ""  # empty string if no traits

            if self.filterShipsWithNoFits:
                if fits > 0:
                    if filter_:
                        self.lpane.AddWidget(ShipItem(self.lpane, ship.ID, (ship.nameCn, shipTrait, fits), ship.race, ship.graphicID))
            else:
                if filter_:
                    self.lpane.AddWidget(ShipItem(self.lpane, ship.ID, (ship.nameCn, shipTrait, fits), ship.race, ship.graphicID))
