...
from gui.myinfo import myInfoFrame
...
class MainFrame(wx.Frame):
    ...
    def __init__(self, title="pyfa"):
        ...
        self.notebookBrowsers.AddPage(self.marketBrowser, "市场", image=marketImg, closeable=False)
        self.marketBrowser.splitter.SetSashPosition(self.marketHeight)

        self.shipBrowser = ShipBrowser(self.notebookBrowsers)
        self.notebookBrowsers.AddPage(self.shipBrowser, "装配", image=shipBrowserImg, closeable=False)
        ...
        #firstl = self.read_file("first.txt")
        #if(firstl == '0'):
            #self.write_file("first.txt")
        frame = myInfoFrame()
        frame.Show()
    ...
    def registerMenu(self):
        ...
        #汉化
        self.Bind(wx.EVT_MENU, self.openCN, id=menuBar.aboutCnId)
    ...
    def updateEsiMenus(self, type):
        ...
        menu.SetLabel(menu.ssoLoginId, "角色管理")
    ...
    def optimizeFitPrice(self, event):
        ...
        if fit:
            ...
            self.waitDialog = wx.BusyInfo("请等待...", parent=self)
