...
class BaseName(ViewColumn):
    ...
    def getText(self, stuff):
        ...
        if isinstance(stuff, Drone):
            return "%dx %s" % (stuff.amount, stuff.item.nameCn)
        elif isinstance(stuff, Fighter):
            return "%d/%d %s" % \
                   (stuff.amount, stuff.getModifiedItemAttr("fighterSquadronMaxSize"), stuff.item.nameCn)
            ...
            if stuff.item.group.name in ("Cargo Container", "Secure Cargo Container", "Audit Log Secure Container", "Freight Container"):
                ...
                if capacity:
                    return "{:d}x {} ({} m\u00B3)".format(stuff.amount, stuff.item.nameCn, formatAmount(capacity, 3, 0, 6))
            return "{:d}x {}".format(stuff.amount, stuff.item.nameCn)
